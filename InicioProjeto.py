        ...
        with open('treinos.txt','w') as arquivotxt:
            try:
                ...
                for i in range(qt_rT):
                    arquivotxt.write(f"{i+1}ºTREINO->|Data: {data}|Distância: {distancia}Km|Tempo de duração: {tempo}h|Localização: {localizacao}|Condição climática: {condicoes_climaticas}|")
                    i+=1
            except ValueError:
        ...
        with open('competicoes.txt','w') as arquivotxt:
            try:
                ...
                for i in range(qt_rC):
                    arquivotxt.write(f"{i+1}ªCOMPETIÇÃO->|Data: {data}|Distância: {distancia}Km|Tempo de duração: {tempo}h|Localização: {localizacao}|Condição climática: {condicoes_climaticas}|")
                    i+=1
            except ValueError:
                print("Alguma informação digitada é inválida")
